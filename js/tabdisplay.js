...
TABDISPLAY = {
	...
	, prePopulate: function(){
		if(TABDISPLAY.favicons.length < 1){
			...
			/*for(var faviconCategory in TABDISPLAY.faviconsDefault.local){
				for(var i = 0, fcLength = TABDISPLAY.faviconsDefault.local[faviconCategory].length; i < fcLength; i++){
					DB.addFavicon(chrome.extension.getURL(TABDISPLAY.faviconsDefault.local[faviconCategory][i]), faviconCategory);
				}
			}*/
			for(var faviconCategory in TABDISPLAY.faviconsDefault.web){
				for(var i = 0, fcLength = TABDISPLAY.faviconsDefault.web[faviconCategory].length; i < fcLength; i++){
					DB.addFavicon(TABDISPLAY.faviconsDefault.web[faviconCategory][i], faviconCategory);
				}
			}
	...
	, addFavicon: function(){
		...
		fav = document.getElementById("newFaviconURL").value;

		if(typeof fav != "undefined"){
			DB.addFavicon(fav, "custom");	
		}		
		this.init();
	}	
